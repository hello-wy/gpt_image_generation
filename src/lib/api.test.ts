...
function createOpenAISettings(overrides = {}) {
...
describe('callImageApi', () => {
  ...
  it.each([false, true])(
    ...
    async (codexCli) => {
      ...
      await callImageApi({
        settings: createOpenAISettings({ apiMode: 'responses', codexCli, reasoningEffort: 'high' }),
        prompt: 'prompt',
  ...
  it('records actual params returned on Images API responses in Codex CLI mode', async () => {
    ...
    const result = await callImageApi({
      settings: createOpenAISettings({ codexCli: true }),
      prompt: 'prompt',
      params: { ...DEFAULT_PARAMS },
      inputImageDataUrls: [],
    })
  ...
  it('streams Images API partial images and resolves the final completed image', async () => {
    ...
    const result = await callImageApi({
      settings: {
        ...createOpenAISettings({ streamImages: true, streamPartialImages: 3 }),
      },
      prompt: 'prompt',
      params: { ...DEFAULT_PARAMS },
      inputImageDataUrls: [],
  ...
  it('does not expect revised prompts on official Images API stream completed events', async () => {
    ...
    const result = await callImageApi({
      settings: {
        ...createOpenAISettings({ streamImages: true }),
      },
      prompt: 'prompt',
      params: { ...DEFAULT_PARAMS },
      inputImageDataUrls: [],
  ...
  it('splits Images API streaming into concurrent single-image requests when n is greater than 1', async () => {
    ...
    const result = await callImageApi({
      settings: {
        ...createOpenAISettings({ streamImages: true, streamPartialImages: 1 }),
      },
      prompt: 'prompt',
  ...
  it('streams Responses API partial images and resolves the completed response image', async () => {
    ...
    const result = await callImageApi({
      settings: {
        ...createOpenAISettings({ apiMode: 'responses', streamImages: true, streamPartialImages: 1 }),
      },
      prompt: 'prompt',
      params: { ...DEFAULT_PARAMS },
      inputImageDataUrls: [],
  ...
  it('parses Responses API image result objects in gallery mode', async () => {
    ...
    const result = await callImageApi({
      settings: createOpenAISettings({ apiMode: 'responses' }),
      prompt: 'prompt',
      params: { ...DEFAULT_PARAMS },
      inputImageDataUrls: [],
    })
  ...
  it('keeps Responses API stream output item images when completed response omits result', async () => {
    ...
    const result = await callImageApi({
      settings: {
        ...createOpenAISettings({ apiMode: 'responses', streamImages: true }),
      },
      prompt: 'prompt',
      params: { ...DEFAULT_PARAMS },
      inputImageDataUrls: [],
